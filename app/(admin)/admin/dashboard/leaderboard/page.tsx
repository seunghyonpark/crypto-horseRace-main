...
export default function LeaderboardList() {
  ...
  const rows = requests?.map((item: any, i: number) => {
    return {
        ...
        lostAmount: item.betAmount - item.prizeAmount/2,
        winningAmount: item.prizeAmount/2,
        rateOfReturn: (item.prizeAmount/2 - (item.betAmount - item.prizeAmount/2) ) / item.betAmount * 100,
